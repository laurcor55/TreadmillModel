function h = bottomOnGdp_test
  ...
  possiblePFs = [2, 8, 9, 11];
  observed = zeros(size(possiblePFs));
  totalRepeats = 1000;

  for repeat = 1:totalRepeats

    allPFsBefore = {[1], [2; 3; 1; 1], [2], [1; 1; 5], [0],[],[2; 2; 1; 2; 4; 2],[2; 2; 1; 2; 4; 3], [2; 1], [4; 2], [2; 1; 1]};
    ...
    [allPFsAfter,Subunits, pfLocations] = bottomOnGdp(allPFsBefore,Subunits, GTPchosen, pfLocations);
  ...
  expected = ones(size(possiblePFs)).*totalRepeats./length(possiblePFs);
